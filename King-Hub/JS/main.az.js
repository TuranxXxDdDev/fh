...
function Ricerca() {
    ...
    switch (tipoRicerca) {
        case 1:
            ...
            intestazione.innerHTML = `Səhifə <span id="categoria">${pagina}</span>`;
            break;
        case 2:
            tipoRicerca = 2;
            intestazione.innerHTML = "";
            ...
            fetch("https://www.eporner.com/api/v2/video/search/?page=" + pagina + "&lq=1&format=json&order=latest&per_page=30&query=" + key_word, {
                ...
                .then(result => { stampaCards(result) })
                .catch(error => console.log('Error:', error));
            intestazione.innerHTML = "Axtarısınız - <span id='ricerca'>" + key_word + "</span>";
            break;
        case 3:
            ...
            if (time == "longest") {
                intestazione.innerHTML = "Axtarısınız <span id='ricerca'>Uzun videolar</span>";
            } else {
                intestazione.innerHTML = "Axtarısınız <span id='ricerca'>Qısa Videolar</span>";
            }
...
function CreaHome() {
    ...
    load();
    if (pagina == 1) {
        intestazione.innerHTML = "Ən son buraxılışlar";
        btnPrev.className = "btn btn-outline-warning disabled";
    } else {
        btnPrev.className = "btn btn-outline-warning";
        btnNext.className = "btn btn-outline-warning";
...
function CreaTrending() {
    ...
    cambiaPagina();
    if (pagina == 1) {
        intestazione.innerHTML = ` <h1 id="intestazione"><span><img src="./img/campfire.png" alt="" id="icone"></span>Trend<span><img
